...
class DeepFake:
    ...
    def load_checkpoints(self,config_path, checkpoint_path, device):
    ...
    def find_best_frame(self,source, driving, cpu):
    ...
    def transfer_video(self,source_img, driving_video_url,result_video_url):
        source_image = imageio.imread(source_img)
        reader = imageio.get_reader(driving_video_url)
        ...
        # inpainting, kp_detector, dense_motion_network, avd_network = load_checkpoints(config_path='config/vox-256.yaml',
        #                                                                               checkpoint_path='checkpoints/vox.pth.tar',
        #                                                                               device=device)
        ...
        predictions = self.make_animation(source_image, driving_video, inpainting, kp_detector, dense_motion_network,
                                     avd_network, device=self.device, mode='relative')

        imageio.mimsave(result_video_url, [img_as_ubyte(frame) for frame in predictions], fps=fps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_image", default='./assets/source-image.png', help="path to source image")
    parser.add_argument("--driving_video", default='./assets/driving-square.mp4', help="path to driving temp")
